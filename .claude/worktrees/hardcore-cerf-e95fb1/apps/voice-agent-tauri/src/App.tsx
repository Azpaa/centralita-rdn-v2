...
export default function App() {
  ...
  const applySnapshot = useCallback((snapshot: AgentStateSnapshot | null) => {
    ...
    if (!snapshot) {
      setCalls([]);
      // No snapshot: drop all local mappings to prevent stale parent→local SID entries
      // outliving the session.
      localCallSidByParentSidRef.current.clear();
      setIncomingConferences(new Map());
      return;
    }
    ...
    // Build the set of parent SIDs the backend considers active. Anything in our
    // local ref that is NOT in this set is a stale mapping (backend-side the call
    // is already closed — typically because reconcile self-healed a lost webhook).
    // Leaving stale mappings around causes "ghost busy" state where Tauri keeps a
    // media leg open for a call that no longer exists.
    ...
    const mapping = localCallSidByParentSidRef.current;
    const orphanedLocalSids: string[] = [];
    for (const [parentSid, localSid] of [...mapping.entries()]) {
      if (!activeParentSids.has(parentSid)) {
        mapping.delete(parentSid);
        if (localSid) orphanedLocalSids.push(localSid);
      }
    }
    // Fire-and-forget disconnect of any local media legs that mapped to parents
    // the backend no longer considers active. Without this, Twilio.Device keeps
    // the audio session open and the softphone stays "busy" silently.
    if (orphanedLocalSids.length > 0) {
      void (async () => {
        for (const localSid of orphanedLocalSids) {
          try {
            await voiceRef.current?.disconnectCall(localSid);
          } catch {
    ...
    // Also drop conference entries for calls no longer active.
    setIncomingConferences((prev) => {
      let changed = false;
      const next = new Map(prev);
      for (const existingSid of [...next.keys()]) {
        if (!activeParentSids.has(existingSid)) {
          next.delete(existingSid);
          changed = true;
        }
      }
      for (const call of snapshot.active_calls) {
  ...
  const executeCallAction = useCallback(async (
    ...
    if (action === 'accept') {
      ...
      // Only stop the ringtone once the conference join succeeded,
      // to avoid silencing the phone when the user didn't actually connect.
      stopIncomingRingtone();
      const localCallSid = result.data?.call_sid;
      if (typeof localCallSid === 'string' && localCallSid.length > 0) {
        localCallSidByParentSidRef.current.set(callSid, localCallSid);
    ...
    if (action === 'hangup') {
      // Order matters: call the backend FIRST so we know the server side accepted the
      // hangup before we tear down local state. If we disconnect + delete mapping first
      // and the backend request fails, the parent call stays active in Twilio/backend
      // but Tauri has lost its mapping, leaving no way to re-sync short of a snapshot.
      ...
      // Backend confirmed hangup. Now tear down local media and mapping.
